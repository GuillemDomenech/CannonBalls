...
class InfiniteScrollingBackground {
     ...
     */
    init?(images : [UIImage], scene : SKScene, scrollDirection : ScrollDirection, transitionSpeed : Double, positionY: CGFloat) {
        ...
        self.sprites = InfiniteScrollingBackground.createSpriteNodes(from: images, spriteSize, positionY: positionY)
        self.scene = scene
    ...
    private func scrollToTheRight() {
        ...
        for index in 0...numberOfSprites - 1 {
            sprites[index].position = CGPoint(x: sprites[index].size.width/2 - (CGFloat(index) * sprites[index].size.width), y: positionY)
     ...
     */
    private static func createSpriteNodes(from images : [UIImage], _ size : CGSize, positionY: CGFloat) -> [SKSpriteNode] {
        var tempSprites = [SKSpriteNode]()
        for image in images {
            let texture = SKTexture(image: image)
            let sprite = SKSpriteNode(texture: texture, color: .clear, size: size)
            sprite.position = CGPoint(x: sprite.position.x, y: positionY)
